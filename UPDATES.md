...
For installations before 0.17.0, remove the old Hermes Jr. plugin from every profile and finish any pending Hermes Agent update before installing the new plugin. The old plugin directory contains a `pyproject.toml`; multiple enabled copies cause Hermes' new package manager to reject the shared workspace. The previous wheel installer cannot migrate those copies safely. See [INSTALL.md](INSTALL.md) for a fresh installation. Do not delete companion state or pairings unless you intend to reset them.
